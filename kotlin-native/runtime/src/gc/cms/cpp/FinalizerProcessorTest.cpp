...
TEST_F(FinalizerProcessorTest, RemoveObject) {
    RunInNewThread([this] {
        ...
        processor.ScheduleTasks(std::move(queue));
        state.finish(1);
        state.waitEpochFinalized(1);
        ASSERT_EQ(threadsCount(), 2);
        ASSERT_TRUE(processor.IsRunning());
        processor.StopFinalizerThreadForTests();
        ASSERT_EQ(threadsCount(), 1);
    });
}
...
TEST_F(FinalizerProcessorTest, ScheduleTasksWhileFinalizing) {
    RunInNewThread([this] {
        ...
        for (int epoch = 0; epoch < epochs; epoch++) {
            state.start(epoch + 1);
            processor.ScheduleTasks(std::move(queues[epoch]));
            state.finish(epoch + 1);
        }
        for (int epoch = 0; epoch < epochs; epoch++) {
            state.waitEpochFinalized(epoch + 1);
        }
        ASSERT_EQ(threadsCount(), 2);
        ASSERT_TRUE(processor.IsRunning());
        processor.StopFinalizerThreadForTests();
